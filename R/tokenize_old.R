...
tokenize_old <- function(strings, orthography.profile = NULL
                     , transliterate = FALSE
  ...
  if (is.null(orthography.profile)) {
    ...
    graphs  <- write.orthography.profile_old(strings, info = FALSE)
    profile <- list(graphs = graphs, rules = NULL)    
  } else {
    # read profile from file
    if (context) {
      profile <- read.orthography.profile_old(orthography.profile
                                          , graphemes
                                          , replacements
                                          , left.context = "left"
                                          , right.context = "right")  
    ...
    if(is.null(profile$graphs)) {
      profile$graphs <- write.orthography.profile_old(strings, info = FALSE)
    }
  } 
  ...
  if (transliterate) {
    profile <- write.orthography.profile_old(strings, replacements = FALSE
                                         , sep = sep, info = TRUE)
  } else {
    profile <- write.orthography.profile_old(strings, replacements = TRUE
                                         , sep = sep, info = TRUE)
  }
